...
// Catalog returns lightweight skill summaries for routers and UI selection.
func (r *Registry) Catalog(includeHidden bool) []SkillSummary {
	r.mu.RLock()
	defer r.mu.RUnlock()

	var skills []SkillSummary
	for _, s := range r.skills {
		if !includeHidden && !s.VisibleInCatalog() {
			continue
		}
		skills = append(skills, s.Summary())
	}
	return skills
}
