...
func TestExternalToolFlattensSuccessDataForReferenceResolution(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"success":true,"data":{"report_path":"E:/bid/out/analysis.md"},"stderr":""}`))
	}))
	defer server.Close()
	...
	if data["report_path"] != "E:/bid/out/analysis.md" {
		...
		t.Fatalf("report_path should be top-level for downstream references, got %s", encoded)
	}
}
