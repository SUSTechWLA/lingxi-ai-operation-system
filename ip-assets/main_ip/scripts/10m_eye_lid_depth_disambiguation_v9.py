...
CHECKPOINT = os.path.join(ROOT, "checkpoints", "sloth_089_eye_bugfix_candidate_v9.blend")
REPORT = os.path.join(ROOT, "reports", "eye_bugfix_candidate_v9.json")
RENDER_DIR = os.path.join(ROOT, "renders", "eye_bugfix", "candidate_v9")
...
def lid_coords(side, part, pose):
    ...
    for col in range(COLS):
        ...
        raw_outer_y = -0.2470 - 0.0005 * s
        outer_y = inner_y + (raw_outer_y - inner_y) * taper
        if part == "Upper" and pose in {"Basis", "Wide"}:
...
for side in ("L", "R"):
    ...
    if tear and tear.type == "CURVE":
        for spline in tear.data.splines:
            for bp in spline.bezier_points:
                bp.co.y = -0.2530
        tear.data.update_tag()
...
def render_view(filename, location, lens=100.0, target=(0.0, 0.0, 2.18)):
...
try:
    ...
    pass
render_view("cycles_face_closeup.png", (0.0, -4.63, 2.30), lens=100.0, target=(0.0, -0.02, 2.22))
...
if not report["invariants_pass"]:
    raise RuntimeError("Candidate v9 invariants failed")
with open(REPORT, "w", encoding="utf-8") as f:
    json.dump(report, f, ensure_ascii=False, indent=2)
bpy.ops.wm.save_as_mainfile(filepath=CHECKPOINT, copy=False)
print(json.dumps({
    "status": "PASS", "checkpoint": CHECKPOINT, "report": REPORT,
    "invariants_pass": report["invariants_pass"], "lid_driver_count": lid_driver_count,
    "head_vertex_count": len(head.data.vertices), "renders": report["renders"],
}, ensure_ascii=False))
