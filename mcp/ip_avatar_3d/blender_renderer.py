...
def _cluster_hand_vertices(
    ...
    for _ in range(24):
        ...
        for record in records:
            ...
            cluster_index = min(
                range(3),
                key=lambda index: (feature - centers[index]).length_squared,
            )
            clusters[cluster_index].append(record)
        for index, cluster in enumerate(clusters):
            if cluster:
                centers[index] = sum(
                    (Vector((float(record["world"].y), float(record["world"].z))) for record in cluster),
                    Vector((0.0, 0.0)),
                ) / len(cluster)
    ...
    thumb_index = min(range(3), key=lambda index: centers[index].y)
    remaining = [index for index in range(3) if index != thumb_index]
    remaining.sort(key=lambda index: centers[index].x)
    return [clusters[remaining[0]], clusters[remaining[1]], clusters[thumb_index]]


def _subdivide_weighted_hand_regions(
    objects: list[bpy.types.Object],
    hand_names: tuple[str, ...],
    *,
    minimum_weight: float = 0.05,
) -> dict[str, int]:
    """Add pre-skinning samples only where the source hand groups deform."""
    ...
    for obj in objects:
        ...
        selected = {
            ...
            for vertex in obj.data.vertices
            if any(
                assignment.group in group_indices and assignment.weight >= minimum_weight
                for assignment in vertex.groups
            )
        }
        if len(selected) < 24:
        ...
        bpy.ops.mesh.subdivide(number_cuts=1, smoothness=0.0, ngon=False)
        bpy.ops.object.mode_set(mode="OBJECT")
        touched += 1

    after = sum(len(obj.data.vertices) for obj in objects if obj.type == "MESH")
    return {
        "handDetailObjectCount": touched,
        "handDetailVertexCountBefore": before,
        "handDetailVertexCountAfter": after,
        "handDetailAddedVertices": max(0, after - before),
    }
...
def enhance_existing_presenter_rig(
    ...
    required_hands = {"hand_l", "hand_r"}
    if not required_hands.issubset(bone_map):
    ...
    if all(role in bone_map for role in ("finger_1_l", "finger_2_l", "finger_3_l", "finger_1_r", "finger_2_r", "finger_3_r")):
        stats["fingerRigEnhanced"] = False
        stats["fingerEnhancementReason"] = "source rig already contains finger roles"
        return stats, bone_map

    hand_detail = _subdivide_weighted_hand_regions(
        objects,
        (bone_map["hand_l"], bone_map["hand_r"]),
    )
    stats.update(hand_detail)

    side_payloads: dict[str, dict[str, Any]] = {}
    for suffix, role in (("L", "hand_l"), ("R", "hand_r")):
        hand_name = bone_map[role]
        ...
        direction = (hand_tail - hand_head).normalized()
        length = max((hand_tail - hand_head).length, float(dimensions["width"]) * 0.06)
        start_projection = length * 0.46
        records: list[dict[str, Any]] = []
        for obj in objects:
            ...
            for vertex in obj.data.vertices:
                ...
                projection = (world - hand_head).dot(direction)
                if projection < start_projection:
                    continue
                records.append(
                    {
                        "object": obj,
                        ...
                        "handWeight": float(assignment.weight),
                    }
                )
        if len(records) < 60:
            raise RuntimeError(f"not enough source hand vertices to create finger rig: {hand_name}")
        clusters = _cluster_hand_vertices(records)
        digits: list[dict[str, Any]] = []
        for digit_index, cluster in enumerate(clusters, start=1):
            projections = [float(record["projection"]) for record in cluster]
            minimum = min(projections)
            maximum = max(projections)
            span = max(maximum - minimum, length * 0.25)
            proximal_points = [record["world"] for record in cluster if record["projection"] <= minimum + span * 0.28]
            distal_points = [record["world"] for record in cluster if record["projection"] >= maximum - span * 0.16]
            base = sum(proximal_points, Vector()) / max(1, len(proximal_points))
            tip = sum(distal_points, Vector()) / max(1, len(distal_points))
            joint = base.lerp(tip, 0.54)
            digits.append(
                {
                    "index": digit_index,
                    "records": cluster,
    ...
    try:
        for suffix, payload in side_payloads.items():
            parent = armature.data.edit_bones[payload["handName"]]
            for digit in payload["digits"]:
                proximal_name = f"Finger_{digit['index']:02d}_Proximal.{suffix}"
                distal_name = f"Finger_{digit['index']:02d}_Distal.{suffix}"
                proximal = armature.data.edit_bones.new(proximal_name)
                proximal.head = world_to_armature @ digit["base"]
                proximal.tail = world_to_armature @ digit["joint"]
                proximal.parent = parent
                proximal.use_connect = False
                proximal.use_deform = True
                distal = armature.data.edit_bones.new(distal_name)
                distal.head = proximal.tail
                distal.tail = world_to_armature @ digit["tip"]
                distal.parent = proximal
                distal.use_connect = True
                distal.use_deform = True
                added_names.extend((proximal_name, distal_name))
    finally:
        bpy.ops.object.mode_set(mode="OBJECT")

    blended_vertices = 0
    for suffix, payload in side_payloads.items():
        hand_name = payload["handName"]
        digits = payload["digits"]
        group_names = [
            name
            for digit in digits
            for name in (
                f"Finger_{digit['index']:02d}_Proximal.{suffix}",
                f"Finger_{digit['index']:02d}_Distal.{suffix}",
            )
        ]
        for obj in objects:
            for group_name in group_names:
                obj.vertex_groups.get(group_name) or obj.vertex_groups.new(name=group_name)

        centers = [digit["featureCenter"] for digit in digits]
        separations = [
            (centers[left] - centers[right]).length
            for left in range(len(centers))
            for right in range(left + 1, len(centers))
        ]
        sigma = max(min(separations, default=0.01) * 0.62, 1e-5)
        records = [record for digit in digits for record in digit["records"]]
        for record in records:
            obj = record["object"]
            vertex_index = int(record["vertexIndex"])
            feature = Vector((float(record["world"].y), float(record["world"].z)))
            distances = [(feature - center).length for center in centers]
            closest_index = min(range(len(digits)), key=lambda index: distances[index])
            closest_digit = digits[closest_index]
            closest_denominator = max(
                float(closest_digit["maximum"] - closest_digit["minimum"]),
                1e-6,
            )
            progress = max(
                0.0,
                min(
                    1.0,
                    (float(record["projection"]) - float(closest_digit["minimum"]))
                    / closest_denominator,
                ),
            )
            smooth_progress = progress * progress * (3.0 - 2.0 * progress)
            finger_total = float(record["handWeight"]) * smooth_progress

            membership_power = 1.6 + progress * 2.8
            raw_memberships = [
                math.exp(-0.5 * (distance / sigma) ** 2) ** membership_power
                for distance in distances
            ]
            membership_total = max(sum(raw_memberships), 1e-8)
            memberships = [value / membership_total for value in raw_memberships]
            if finger_total > 0.08 and sorted(memberships, reverse=True)[1] > 0.08:
                blended_vertices += 1

            for group_name in group_names:
                obj.vertex_groups[group_name].remove([vertex_index])
            obj.vertex_groups[hand_name].add(
                [vertex_index],
                max(0.0, float(record["handWeight"]) - finger_total),
                "REPLACE",
            )
            for digit, membership in zip(digits, memberships):
                denominator = max(float(digit["maximum"] - digit["minimum"]), 1e-6)
                normalized = max(
                    0.0,
                    min(
                        1.0,
                        (float(record["projection"]) - float(digit["minimum"])) / denominator,
                    ),
                )
                distal_blend = max(0.0, min(1.0, (normalized - 0.62) / 0.38))
                distal_blend = distal_blend * distal_blend * (3.0 - 2.0 * distal_blend)
                digit_weight = finger_total * membership
                proximal_name = f"Finger_{digit['index']:02d}_Proximal.{suffix}"
                distal_name = f"Finger_{digit['index']:02d}_Distal.{suffix}"
                proximal_weight = digit_weight * (1.0 - distal_blend)
                distal_weight = digit_weight * distal_blend
                if proximal_weight > 1e-6:
                    obj.vertex_groups[proximal_name].add([vertex_index], proximal_weight, "REPLACE")
                if distal_weight > 1e-6:
                    obj.vertex_groups[distal_name].add([vertex_index], distal_weight, "REPLACE")
    ...
    stats.update(
        {
            ...
            "presenterControlsReady": has_presenter_controls(bone_map),
            "fingerRig": True,
            "fingerRigEnhanced": True,
            "fingerBoneCount": len(added_names),
            "addedFingerBones": added_names,
            "fingerWeightingMode": "soft_digit_blend",
...
def retopologize_source_face(
    ...
    *,
    ...
    if exported_face and required_internal_names.issubset(exported_internals):
        ...
        boundary = upper.union(lower)
        exported_face["source_face_topology"] = "integrated_source_retopology"
        exported_face["integrated_mouth_seam"] = True
        exported_face["mouth_seam_edge_count"] = max(0, boundary_edge_count // 2)
        exported_face["mouth_boundary_edge_count"] = boundary_edge_count
        exported_face["mouth_upper_boundary_count"] = len(upper)
        exported_face["mouth_lower_boundary_count"] = len(lower)
        exported_face["mouth_upper_boundary_indices"] = json.dumps(sorted(upper))
        exported_face["mouth_lower_boundary_indices"] = json.dumps(sorted(lower))
        exported_face["mouth_center_x"] = center_x
        exported_face["mouth_center_z"] = center_z
        exported_face["mouth_surface_y"] = (
            sum(float((exported_face.matrix_world @ exported_face.data.vertices[index].co).y) for index in boundary)
            / len(boundary)
            if boundary
            else float(head_bounds["min"].y) + float(head_bounds["depth"]) * 0.10
        )
        exported_face["mouth_radius_x"] = radius_x
        exported_face["head_region_width"] = float(head_bounds["width"])
        exported_face["head_region_depth"] = float(head_bounds["depth"])
        exported_face["source_texture_face_preserved"] = True
        exported_face["facial_topology_mode"] = "source_retopology"
        return {
            "mouth": exported_face,
    ...
    for frame in range(1, frame_end + 1, 2):
        ...
        if mouth:
            if mouth.data.shape_keys:
                ...
                if key_blocks.get("Eye_Blink.L") and key_blocks.get("Eye_Blink.R"):
                    blink_amount = max(
                        (
                    ...
                    rich_values = {
                        "Eye_Blink.L": blink_amount,
                        "Eye_Blink.R": blink_amount,
...
def create_action_library(
    ...
    if mouth and mouth.type == "MESH" and mouth.data.shape_keys:
        ...
        face_specs: dict[str, list[tuple[int, dict[str, float]]]] = {
            "Face_Neutral": [(1, {}), (end, {})],
            "Face_Happy": [
                (1, {"Mouth_Smile": 0.72, "Cheek_Smile.L": 0.72, "Cheek_Smile.R": 0.72, "Eye_Blink.L": 0.08, "Eye_Blink.R": 0.08}),
                (end, {"Mouth_Smile": 0.72, "Cheek_Smile.L": 0.72, "Cheek_Smile.R": 0.72, "Eye_Blink.L": 0.08, "Eye_Blink.R": 0.08}),
            ],
            "Face_Thinking": [
            ...
            "Face_Blink": [
                (1, {}),
                (max(2, fps // 6), {"Eye_Blink.L": 1.0, "Eye_Blink.R": 1.0}),
                (max(3, fps // 3), {}),
            ],
        }
