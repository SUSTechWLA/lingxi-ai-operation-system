...
- Authored studios should expose `Camera_Wide`, `Camera_Medium`, and `Camera_Close`. `cameraPreset=auto` may cut between all three; `cameraPreset=front_talking` uses a full-body front opening followed by a stable front medium shot so the head and hands stay inside frame. Fixed `wide`, `medium`, `close`, `three_quarter`, and `transition` presets remain available for explicit shots.
