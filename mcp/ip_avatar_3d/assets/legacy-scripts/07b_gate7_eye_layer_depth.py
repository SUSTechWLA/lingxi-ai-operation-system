...
REPORT = os.path.join(ROOT, "reports", "gate7_eye_layer_depth_report.json")
RENDER_DIR = os.path.join(ROOT, "renders", "lookdev", "gate7")


offsets = {"EYE_Iris_": 0.0020, "EYE_Pupil_": 0.0037, "EYE_Catchlight_": 0.0037}
moved = {}
for prefix, offset in offsets.items():
    for side in ("L", "R"):
        obj = bpy.data.objects.get(prefix + side)
        if obj:
            before = list(obj.matrix_world.translation)
            world = obj.matrix_world.copy()
            world.translation.y += offset
            obj.matrix_world = world
            moved[obj.name] = {"before": before, "after": list(obj.matrix_world.translation), "delta_y": offset}
...
checks = {
    "all_six_layers_moved": len(moved) == 6,
...
if not all(checks.values()):
    raise RuntimeError("Gate 7b invariant failed: " + json.dumps(checks))
bpy.ops.wm.save_as_mainfile(filepath=CHECKPOINT)
with open(REPORT, "w", encoding="utf-8") as handle:
    json.dump({"gate": "7b", "status": "PASS", "moved_layers": moved, "checks": checks, "renders": outputs}, handle, ensure_ascii=False, indent=2)
