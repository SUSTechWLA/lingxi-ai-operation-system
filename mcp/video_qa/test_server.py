...
class VideoQAServerTests(unittest.TestCase):
    ...
    def test_shot_report_maps_text_gate_to_rerender_html(self) -> None:
        ...
        self.assertEqual(reports[0]["repairPlan"]["targetShotId"], "SHOT_TEXT")
        self.assertEqual(reports[0]["repairPlan"]["sourceCandidateId"], "candidate_01")
        self.assertEqual(reports[0]["repairPlan"]["attemptIndex"], 1)
        self.assertTrue(reports[0]["repairPlan"]["preserve"])
        self.assertIn("prompt_alignment", reports[0]["repairPlan"]["lockedDimensions"])
        self.assertIn("text_intent", reports[0]["repairPlan"]["repairTargets"])
        self.assertEqual(reports[0]["repairPlan"]["nextToolCall"], "RERENDER_HTML")
