...
if ! docker info &> /dev/null; then
    ...
    wait_for_docker || { error "Docker 启动失败，请手动打开 Docker Desktop"; exit 1; }
fi
...
info "7.5 模块间通信测试"
if [ -n "$TEST_TASK_ID" ]; then
    CTX_COUNT=$(curl -s "http://localhost:8082/api/task/${TEST_TASK_ID}/context" | python3 -c "import sys, json; d=sys.stdin.read(); print(len(json.loads(d)) if d else 0)" 2>/dev/null || echo "0")
    if [ "$CTX_COUNT" -ge 0 ]; then
        echo -e "  ${GREEN}✓${NC} 通信正常 (上下文记录数: $CTX_COUNT)"
        passed=$((passed + 1))
    else
        echo -e "  ${RED}✗${NC} 通信异常"
        failed=$((failed + 1))
