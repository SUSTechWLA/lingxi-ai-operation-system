# Skill：灵犀AIOS 自媒体运营功能全量升级（基于现有架构扩展）

## 一、Skill 概述

本 Skill 用于指导在灵犀AI OS 现有架构上，通过**复用并扩展现有的 Publish 模块、Orchestrator、Worker、Tool 接口和事件系统**，从当前的「单次发布」能力升级为覆盖「素材管理 → 内容生成 → 对话创作 → 运营配套」的自媒体全链路能力。所有新增功能遵循现有分层规范、API 规范和扩展点机制。
