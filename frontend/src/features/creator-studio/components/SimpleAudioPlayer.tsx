...
import {
  ...
  resetMediaTransportState,
  secondsToIntegerMilliseconds,
} from '../mediaRange'
...
export default function SimpleAudioPlayer({
  ...
  useEffect(() => {
    const reset = resetMediaTransportState()
    setIsPlaying(reset.isPlaying)
    setCurrentTime(reset.currentTime)
    setDuration(reset.duration)
    setVolume(reset.volume)
    setIsMuted(reset.isMuted)
    setPlaybackRate(reset.playbackRate)
    setFailed(false)
    setRetries(0)
  }, [src])
  ...
  }, [currentTime, duration, isPlaying, onPlaybackStateChange, retries, src])
  ...
  const handleError = () => {
    const failure = audioPlaybackFailure(retries)
    setFailed(true)
    setIsPlaying(false)
    if (!failure.playbackAvailable) onError?.()
  }
    ...
    return (
      <div className="simple-audio-player is-error" role="group" aria-label={title}>
        <div className="simple-audio-error" role="alert">
          ...
          <div>
            {failure.canRetry && <button type="button" className="creator-secondary-button" onClick={retryPlayback}>重新加载预览</button>}
  ...
  return (
    <div className="simple-audio-player" role="group" aria-label={title} tabIndex={0} onKeyDown={handleKeyboard}>
      ...
      <div className="simple-video-controls simple-audio-controls">
        <button type="button" className="simple-video-icon-button simple-audio-play" aria-label={isPlaying ? '暂停' : '播放'} onClick={togglePlayback}>
          <span aria-hidden="true">{isPlaying ? '❚❚' : '▶'}</span>
        </button>
        <span className="simple-video-time simple-audio-time" aria-live="off">{formatTime(currentTime)} / {formatTime(duration)}</span>
        <input
          className="simple-video-seek simple-audio-seek"
          type="range"
          min="0"
          max={duration || 0}
          step="0.05"
        ...
        />
        <button type="button" className="simple-video-icon-button simple-audio-mute" aria-label={isMuted ? '取消静音' : '静音'} onClick={toggleMute}>
          <span aria-hidden="true">{isMuted || volume === 0 ? '⌁' : '◖'}</span>
        </button>
        <input
          className="simple-video-volume simple-audio-volume"
          type="range"
          min="0"
          max="1"
          step="0.05"
        ...
        />
        <select className="simple-video-rate simple-audio-rate" value={playbackRate} aria-label="播放速度" onChange={event => changePlaybackRate(Number(event.target.value))}>
        ...
        <a className="simple-video-download simple-audio-download" href={src} download={downloadName || true} aria-label="下载音频">下载</a>
      </div>
